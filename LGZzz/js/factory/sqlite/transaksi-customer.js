...
.factory('TransCustLiteFac',function($rootScope,$http, $q, $filter, $window,$cordovaSQLite,UtilService)
{
    ...
    var UpdateIsOnServer = function (NOMOR_TRANS)
    {
        var deferred        = $q.defer();
        var NOMOR_TRANS     = NOMOR_TRANS;
        var IS_ONSERVER     = 1;
        var updatestatusbuy = 'UPDATE Tbl_CustBuyTrans SET IS_ONSERVER = ? WHERE NOMOR_TRANS = ?';
        $cordovaSQLite.execute($rootScope.db,updatestatusbuy,[IS_ONSERVER,NOMOR_TRANS])
        .then(function(result) 
        {
            deferred.resolve(result);
        },
