...
.factory('BarangForSaleLiteFac',function($rootScope,$http, $q, $filter, $window,$cordovaSQLite,UtilService)
{
    var SetBarangForSale = function (datatosave)
    {
        ...
        var isitable            = [TGL_SAVE,ITEM_ID,ITEM_NM,ITEM_HARGA,STOCK_MAX,GAMBAR,FORMULA];
        var queryinsertbarangpenjualan  = 'INSERT INTO Tbl_BarangPenjualan (TGL_SAVE,ITEM_ID,ITEM_NM,ITEM_HARGA,STOCK_MAX,GAMBAR,FORMULA) VALUES (?,?,?,?,?,?,?)';
    ...
    {
        var deferred = $q.defer();
        var queryselectinvcheck = 'SELECT * FROM Tbl_BarangPenjualan WHERE TGL_SAVE = ?';
